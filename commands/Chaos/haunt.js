...
export const data = new SlashCommandBuilder()
    .setName('haunt')
    .setDescription('Use /haunt on someone.')
...
export async function execute(interaction) {
    ...
    if (11 > 0 && score < 11) {
        return interaction.reply({
            embeds: [buildEmbed('chaos', pick(replies['haunt'].failure)(sender))],
            ephemeral: true,
        });
    }
    ...
    const text = backfired
        ? pick(replies['haunt'].backfire)(sender, target.id, weapon)
        : pick(replies['haunt'].success)(sender, target.id, weapon);
