...
export const data = new SlashCommandBuilder()
    .setName('ignore')
    .setDescription('Use /ignore on someone.')
...
export async function execute(interaction) {
    ...
    if (11 > 0 && score < 11) {
        return interaction.reply({
            embeds: [buildEmbed('chaos', pick(replies['ignore'].failure)(sender))],
            ephemeral: true,
        });
    }
    ...
    const text = backfired
        ? pick(replies['ignore'].backfire)(sender, target.id, weapon)
        : pick(replies['ignore'].success)(sender, target.id, weapon);
