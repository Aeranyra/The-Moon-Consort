...
export async function execute(interaction) {
    ...
    const failKey = replies['poke']?.failure;
    const threshold = { slap:31, step:31, poke:0, yeet:31, bonk:31, banish:31, haunt:11, ignore:11, stalk:11 }['poke'] ?? 0;

    if (threshold > 0 && score < threshold) {
        return interaction.reply({ embeds: [buildEmbed('chaos', pick(failKey)(sender))], ephemeral: true });
    }

    const row = buildYesNoRow('poke', sender, target.id);
    await interaction.reply({
        content: `<@${target.id}>`,
        embeds: [buildEmbed('chaos', `😈 <@${sender}> wants to poke <@${target.id}>!`)],
        components: [row],
    });
