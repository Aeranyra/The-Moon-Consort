...
export const data = new SlashCommandBuilder()
    .setName('step')
    .setDescription('Use /step on someone.')
...
export async function execute(interaction) {
    ...
    if (31 > 0 && score < 31) {
        return interaction.reply({
            embeds: [buildEmbed('chaos', pick(replies['step'].failure)(sender))],
            ephemeral: true,
        });
    }
    ...
    const text = backfired
        ? pick(replies['step'].backfire)(sender, target.id, weapon)
        : pick(replies['step'].success)(sender, target.id, weapon);
