...
export async function execute(interaction) {
    ...
    if (sender === target) {
        return interaction.reply({
            embeds: [buildEmbed('chaos', '🌙 You cannot yeet yourself. That\'s just falling.')],
            ephemeral: true,
        });
    }
    ...
    if (score < BOND_THRESHOLD) {
        return interaction.reply({
            embeds: [buildEmbed('chaos', pick(replies.yeet.failure)(sender))],
            ephemeral: true,
        });
    }
    ...
    const text = backfired
        ? pick(replies.yeet.backfire)(sender, target, weapon)
        : pick(replies.yeet.success)(sender, target, weapon);
