...
export async function execute(interaction) {
    ...
    if (score < 11) {
        return interaction.reply({ embeds: [buildEmbed('affection', pick(replies.cuddle.failure)(sender))], ephemeral: true });
    }

    const row = buildYesNoRow('cuddle', sender, target.id);
    await interaction.reply({
        content: `<@${target.id}>`,
        embeds: [buildEmbed('affection', pick(replies.prompts['cuddle'])(sender, target.id), { title: '...' })],
        components: [row],
    });
