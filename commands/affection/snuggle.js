...
export async function execute(interaction) {
    ...
    if (score < 11) {
        return interaction.reply({ embeds: [buildEmbed('affection', pick(replies.snuggle.failure)(sender))], ephemeral: true });
    }

    const row = buildYesNoRow('snuggle', sender, target.id);
    await interaction.reply({
        content: `<@${target.id}>`,
        embeds: [buildEmbed('affection', pick(replies.prompts['snuggle'])(sender, target.id), { title: '...' })],
        components: [row],
    });
