...
export async function execute(interaction) {
    ...
    if (score < 31) {
        return interaction.reply({ embeds: [buildEmbed('fun', pick(replies['punish'].failure)(sender))], ephemeral: true });
    }

    const row = buildYesNoRow('punish', sender, target.id);
    await interaction.reply({
        content: `<@${target.id}>`,
        embeds: [buildEmbed('fun', `😈 <@${sender}> wants to punish <@${target.id}>!`)],
        components: [row],
    });
