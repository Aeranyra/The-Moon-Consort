...
export const data = new SlashCommandBuilder()
    .setName('punish')
    .setDescription('Use /punish on someone.')
...
export async function execute(interaction) {
    ...
    if (target.id === sender) return interaction.reply({
        embeds: [buildEmbed('fun', '🌙 You cannot do this to yourself.')],
        ephemeral: true,
    });
    ...
    const text = backfired
        ? pick(replies['punish'].backfire)(sender, target.id, weapon)
        : pick(replies['punish'].success)(sender, target.id, weapon);
