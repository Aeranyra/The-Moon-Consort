...
export async function execute(interaction) {
    ...
    if (sender === target) return interaction.reply({ content: '🌙 You cannot send a butterfly to yourself.', ephemeral: true });
    ...
    if (score < 31) {
        return interaction.reply({ content: pick(replies.butterfly.failure)(sender), ephemeral: true });
    }

    const transferred = await transferButterfly(sender, target, guildId);
    if (!transferred) {
        return interaction.reply({ content: pick(replies.butterfly.empty)(sender), ephemeral: true });
    }

    await interaction.reply({ content: pick(replies.butterfly.success)(sender, target) });
}
